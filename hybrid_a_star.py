...
def main():
    ...
    if show_animation:
        ...
        # plot the vertical lines with the resolution of 2.0
        line_x = np.arange(0, 61, XY_GRID_RESOLUTION)
        line_y = np.arange(0, 61, XY_GRID_RESOLUTION)
        for i in range(len(line_x)):
            plt.plot([line_x[i], line_x[i]], [0, 60], "c", linewidth=0.5)
            plt.plot([0, 60], [line_y[i], line_y[i]], "c", linewidth=0.5)
        plt.axis("equal")
